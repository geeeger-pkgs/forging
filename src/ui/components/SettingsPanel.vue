...
<template>
  <div class="settings">
    ...
    <section class="card">
      <h3>视听与手感</h3>
      <p class="dim">
        音效为程序化合成（零外部资源，不增加加载体积）。音效与特效是两个独立开关：
      ...
      <p class="dim">
        说明：「关闭」只关闭覆盖层的粒子与飘字，顶部提示条、面板文字与场景动画照常（关键信息始终以文字给出，不会因关动效而丢失）；
        系统开启「减少动态效果」时，「自动」档会退化为「简化」。
      </p>
      <p class="dim">
        已知平台限制：iOS 处于静音档时 WebAudio 不发声（系统行为，无法绕过）；浏览器要求首次点击/按键后音效才会启用；
        页面切到后台时自动静音，回到前台恢复。
      </p>
    </section>
