...
动作队列（可扩容；完成/停止/中断后自动接续；不可行的队列项自动跳过并提示）、无限模式、音效/特效档位与减少动效偏好。

---

## 存档：格式与三条通道（v3.8.0）

存档在本地（`localStorage` 主槽 + 备份槽，双槽损坏自动回退），**导出/导入**用的是同一份压缩 + 加密文本：

```
FGS1:<base64>   =   iv(12B) ‖ AES-GCM( flags(1B) ‖ [gzip]( UTF-8 JSON ) )
```

- **压缩**：`CompressionStream('gzip')`，不可用时自动降级为不压缩（老浏览器不会空手而归）。
  实测：3053 字符的明文存档 → 2153 字符的 `FGS1` 串（约 −30%，越到后期收益越大）。
- **加密**：AES-GCM，密钥内置。**目的是防明文与防误改，不是对抗性安全**——谁都能解开，但
  **改动一个字符**就会在解密时校验失败并明确报错（GCM 认证标签即篡改检测），比明文 JSON 更难被"手滑改坏"。
- **兼容**：导入仍接受旧版明文 JSON（历史导出文件不作废）；解析失败/版本高于当前一律拒绝并提示。

三条通道（移动端常无法下载或上传文件，故剪贴板与粘贴框是并列的一等路径）：

| 通道 | 入口 | 适用 |
|---|---|---|
| 文件 | 「导出文件」/「导入文件」（`forging-save-YYYYMMDD.fgs.txt`） | 桌面、可下载/选文件的环境 |
| 剪贴板 | 「复制到剪贴板」/「从剪贴板读取」 | 手机、受限 WebView（无文件 API） |
| 粘贴框 | 文本框 + 「从文本导入」 | 所有环境的兜底；从剪贴板读入后也可人工核对 |

实机验收记录（v3.8.0）：剪贴板导出 → 用 **Node 的 `node:crypto` 独立重写解码器**解回 JSON 且字段一致
（跨实现交叉验证）；清档后用**粘贴导入**还原为原角色（createdAt / 金币 / 矿石一致）；
「导出文件」触发真实下载事件且文件名为 `forging-save-YYYYMMDD.fgs.txt`。

**顺带修掉的缺陷**：「清档重来」过去会 **静默失效** —— `clearSave()` 之后 `location.reload()` 触发
`beforeunload` 兜底保存，把内存里的旧档又写回刚清空的槽（表现："点了清档，进度还在"）。
现在清档路径先 `suppressAutosave()`，并有 jsdom 回归用例 + 反证探针（注释掉该行即变红）。
...
src/app    # 壳层：store / 持久化（存档 v15）/ 存档编解码 save-codec（FGS1）/ describe（弹窗口径）
...
- **测试**：568 用例 / 34 文件全绿（含内核数值、存档迁移、存档编解码、组件可达性、守卫与契约）
...
| 前缀 | 内容 | 入口 |
|---|---|---|
| `design-*` | 各版本设计定稿（范围 / 证据要求 / 验收） | `design-v3.8.md`（存档通道：格式 / 威胁模型 / 验收） |
...
当前 **3.8.0**（v3.5.0 封板后转入 UI/UX 打磨线，版本号逐次跟随迭代：全局视觉升级、移动端交互修复、桌面右栏分区）。
