...
## 1.5 传承经济终稿（3.4.1 终审订正）

| 量 | 值 | 证据 |
|---|---|---|
| 点数曲线 | **steps² + steps**（steps = ⌊(min−40)/10⌋）：min 50/60/70/80/90/100 → **2/6/12/20/30/42** | prestigePointsFor 与 D 段 stretchScan 同式 |
| 首点门槛 | 最低技能 Lv50 = **2763.6h**（旧口径"总 120 得 12 点"= 321.7h） | sim-audit 的 capped 口径 |
| 满级一轮 | min 100 → **42 点 / 31342h**（保守上界；实际更快） | 同上 |
| perk 树 | 基础 78 点 → **1.86 轮**；含深造 234 点 → **5.57 轮** | perks.json Σcost×max；过载 = 2×Σ |
| 点/小时 | 随最低技能**严格单调递增**（0.00072→0.00134），满级唯一最优（ratio 1.00） | stretchScan |

> **口径订正（终审要求）**：旧文写的"8.59×"是拿**新的 1 点档**比**旧的 12 点档**，基准不成立，已废除。
> 正确表述：新口径首点（2 点）代价 2763.6h；与旧口径"总 120 即得 12 点（321.7h）"相比，**同点数（12 点）的代价为 10310h**。
> **已知取舍**：任一提前停点的点/小时均低于满级（设计使然——点数只依赖最低技能）；早期传承换的是**即时**的小额精通，
> 而非效率最优。该取舍随 v3.5 发布说明一并声明。
