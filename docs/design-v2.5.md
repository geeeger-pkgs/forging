...
- 性能预算（硬性，见 §3 实测）：**总粒子上限沿用既有 MAX_P = 260**（其中场景本身的常驻粉尘占多数）；**交互爆发**每次 ≤ 60 粒且 ≤ 4 次/秒；飘字 ≤ 6 条；单帧绘制 ≤ **1.5ms**（1440px 桌面）；主循环（250ms tick）额外耗时 ≤ **0.5ms**（仅音效触发与事件入队）。
  > 口径说明：设计初稿曾写「同屏粒子 ≤120」，与既有实现（ 的 ）冲突 —— 已按既有实现校正（评审教训：文档数字必须与实现一致）。
- 降级：`fx = reduced` 时关闭常驻粉尘与交互爆发、只保留飘字与光环；`fx = off` 时完全不绘制表现层（rAF 仍驱动场景本身，但跳过粒子/飘字/光环的计算与绘制）。
...
| 12 | `data/fx.json`（新，生成器产出） | 音效清单（16 条含音色参数）+ 预算常量 + 设置默认值 —— 供内核/UI/证据脚本**同源读取**（避免脚本去解析 TS 常量） |
| 13 | `scripts/audit-fx.mjs`（新） | 输出：音效清单与覆盖、事件→表现映射完整性、动效预算常量、设置矩阵（**本版的"证据脚本"**） |
...
| E3 | 动效预算常量 | 总粒子 ≤260（既有 MAX_P）/ 交互爆发 ≤60·≤4 次每秒 / 飘字 ≤6 / 单帧 ≤1.5ms —— **常量集中放 data/fx.json**，脚本直读断言 |
