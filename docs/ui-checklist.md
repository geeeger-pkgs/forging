...
## 7. 实机验证的存档纪律（v3.7.20 教训）

为造测试场景**直接改 localStorage 存档**会覆盖玩家真实进度；
而且 `forging.save.bak` 双槽会在随后的自动保存周期里被一并覆盖（来不及回滚）。规则：

1. **先备份**：改存档前先把 `forging.save` 的 JSON 原样存到项目外的文件；
2. **优先不动真实存档**：用独立浏览器 profile，或另起端口/清档的干净实例；
3. 验证完**立刻用备份写回**（赶在下一个自动保存周期前，autosaveSec = 30s）。
